...
def detect_country_code(value: str):
    ...
    # Longest prefix first so 1242 is checked before 1, etc.
    for code in COUNTRY_CODE_LIST:
        if clean.startswith(code) and len(clean) > len(code):
            national = clean[len(code):]

            # Basic E.164 sanity: country code + at least 4 subscriber digits.
            if len(national) < 4:
                return None
...
@app.get("/api/lookup/{number:path}")
def lookup_path(
    number: str,
    key: Optional[str] = Query(None),
    x_api_key: Optional[str] = Header(None, alias="x-api-key"),
...
def direct_lookup(
    ...
    # Avoid treating our own API routes as phone numbers.
    if number.startswith("api/") or number in {"favicon.ico", "robots.txt"}:
